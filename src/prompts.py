...
- done: отметить прием медикамента раньше времени (фразы типа "я принял X", "выпил X", "выпила X", "готово")
- help: запрос помощи или информации о возможностях бота (фразы типа "что ты умеешь", "помощь", "какие команды", "что можешь", "справка", "инструкция")
...
ВАЖНО: Слова "выпил", "выпила", "выпило", "выпили" ВСЕГДА означают отметку приема медикамента (done), независимо от регистра и рода.
...
ВАЖНО О ГРАММАТИКЕ:
- Поле "medication_name" должно содержать название медикамента в НИЖНЕМ РЕГИСТРЕ
- Это нужно для правильного формирования сообщения "Добавлено: героин в 12:00"
- Примеры: "аспирин" (не "Аспирин"), "габапентин" (не "Габапентин"), "ламотриджин" (не "Ламотриджин")
...
ВАЖНО О ГРАММАТИКЕ:
- Поле "medication_name" должно содержать название медикамента в ИМЕНИТЕЛЬНОМ ПАДЕЖЕ (nominative case)
- Это нужно для правильного формирования сообщения "Героин удален из расписания", "Габапентин удален из расписания"
...
- Если понятно какой медикамент (например, один аспирин в 19:00), верни его ID, название медикамента и новое время
...
ВАЖНО О ГРАММАТИКЕ:
- Поле "medication_name" должно содержать название медикамента в РОДИТЕЛЬНОМ ПАДЕЖЕ (genitive case) и в НИЖНЕМ РЕГИСТРЕ
- Это нужно для правильного формирования сообщения "Время приема аспирина изменено", "Время приема ламотриджина изменено"
- Примеры правильных значений: "аспирина" (не "аспирин" и не "Аспирина"), "габапентина" (не "габапентин"), "ламотриджина" (не "ламотриджин")
- ВСЕГДА используй родительный падеж: аспирин → аспирина, габапентин → габапентина, ламотриджин → ламотриджина, парацетамол → парацетамола

Примеры ответов:
- Успешное изменение: {{"status": "success", "medication_name": "аспирина", "medication_id": 1, "new_times": ["19:00", "21:00"]}}
- Успешное изменение (другой пример): {{"status": "success", "medication_name": "габапентина", "medication_id": 2, "new_times": ["10:00"]}}
...
- Если пользователь говорит "у всего аспирина теперь доза 100мг", верни массив со всеми ID аспирина
- Если понятно какой медикамент (например, один аспирин), верни один объект в массиве
- Если непонятно (например, несколько аспиринов с разным временем и не указано "всего"), попроси уточнить

ВАЖНО О ФОРМАТЕ ОТВЕТА:
- ВСЕГДА возвращай массив объектов, даже если изменяется только один медикамент
- Каждый объект должен содержать: medication_id (число), medication_name (строка), new_dosage (строка)
- Если изменяется несколько приёмов одного медикамента, верни отдельный объект для каждого ID

ВАЖНО О ГРАММАТИКЕ:
- Поле "medication_name" должно содержать название медикамента в РОДИТЕЛЬНОМ ПАДЕЖЕ (genitive case) и в НИЖНЕМ РЕГИСТРЕ
- Это нужно для правильного формирования сообщения "Дозировка аспирина изменена", "Дозировка габапентина изменена"
- Примеры правильных значений: "аспирина" (не "аспирин" и не "Аспирина"), "габапентина" (не "габапентин"), "ламотриджина" (не "ламотриджин")
- ВСЕГДА используй родительный падеж: аспирин → аспирина, габапентин → габапентина, ламотриджин → ламотриджина, парацетамол → парацетамола

Примеры ответов:
- Успешное изменение одного: [{{"medication_id": 1, "medication_name": "аспирина", "new_dosage": "300 мг"}}]
- Успешное изменение всех приёмов: [{{"medication_id": 1, "medication_name": "аспирина", "new_dosage": "100 мг"}}, {{"medication_id": 3, "medication_name": "аспирина", "new_dosage": "100 мг"}}, {{"medication_id": 5, "medication_name": "аспирина", "new_dosage": "100 мг"}}]
...
- Если пользователь указал город из списка, верни соответствующее смещение и название города
- Если пользователь указал смещение напрямую (например, "+3" или "-5"), преобразуй в формат "+03:00" или "-05:00", а название города оставь пустым
- Если города нет в списке, попроси указать смещение относительно UTC

Примеры ответов:
- Успешное определение города: {{"status": "success", "timezone_offset": "+03:00", "city_name": "Москва"}}
- Успешное определение смещения: {{"status": "success", "timezone_offset": "+03:00", "city_name": ""}}
...
- Если пользователь говорит "я принял парацетамол" без указания времени, а он принимает его несколько раз в день, верни все ID из доступных
- Если медикамент один, верни один ID
- НЕ ПРИДУМЫВАЙ ID - используй только те, что есть в расписании выше
...
def get_help_command_prompt() -> str:
    ...
    """
    return """Ты ассистент приема медикаментов. Пользователь запросил помощь или информацию о возможностях бота.

Напиши дружелюбное и информативное сообщение на русском языке, которое объясняет:

1. Основное назначение бота: управление расписанием приема медикаментов
2. Доступные команды и возможности:
   - Добавление медикаментов с расписанием (например, "я принимаю аспирин в 10:00")
   - Просмотр текущего расписания (например, "что я принимаю", "покажи расписание")
   - Изменение времени приема (например, "измени время аспирина на 12:00")
   - Изменение дозировки (например, "измени дозировку аспирина на 200 мг")
   - Удаление медикаментов (например, "удали аспирин", "больше не принимаю парацетамол")
   - Отметка о раннем приеме (например, "я принял аспирин")
   - Изменение часового пояса (например, "моя часовая зона Москва")
3. Поощряй использование естественного языка - пользователь может общаться как с человеком

Сообщение должно быть теплым, понятным и мотивирующим. Подчеркни, что бот понимает естественный русский язык и пользователю не нужно запоминать специальные команды.
