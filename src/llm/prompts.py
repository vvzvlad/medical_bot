...
def get_done_command_prompt(user_message: str, schedule: list) -> str:
    ...
    return f"""Ты ассистент приема медикаментов. Пользователь хочет отметить, что принял медикамент раньше времени.
...
Определи, какой медикамент пользователь принял:
- Если пользователь говорит "я принял парацетамол", а он принимает его несколько раз в день, верни все ID
