...
def is_time_to_send_notification(
    ...
    # Check if current time has reached or passed the medication scheduled time.
    # Deduplication (not sending twice) is handled by the caller via database state
    # (reminder_message_id). This ensures notifications are never missed due to
    # scheduler drift or processing delays.
    return (user_now.hour, user_now.minute) >= (med_hour, med_minute)
