...
- `medications` - массив медикаментов
  ...
  - `time_utc` - время приема в формате Unix timestamp (UTC)
  - `last_taken` - timestamp последнего приема или null
...
- Время хранится в UTC и конвертируется при отображении с учетом `timezone_offset`
- История приема не хранится, только текущий статус (`last_taken`)
...
**Список городов и временных зон (расширить таймзонами городв в рф):**
```
Москва: +03:00
